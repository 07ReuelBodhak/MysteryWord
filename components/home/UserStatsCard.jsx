...
export function UserStatsCard({ user }) {
  if (!user) return null;
  console.log("Rendering UserStatsCard with user:", user);
  return (
    <Card className="flex flex-col gap-6 bg-neutral-900/30">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-neutral-400 text-sm">{user.total_points} Points</p>
        </div>
      </div>
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 mt-2">
        <div className="bg-neutral-900 p-3 rounded-xl border border-neutral-800 flex flex-col justify-center">
          <p className="text-[10px] uppercase font-bold tracking-widest text-neutral-500 mb-1 flex items-center gap-1.5">
        ...
        <div className="bg-neutral-900 p-3 rounded-xl border border-neutral-800 flex flex-col justify-center">
          <p className="text-[10px] uppercase font-bold tracking-widest text-neutral-500 mb-1 flex items-center gap-1.5">
            <Trophy className="w-3 h-3" /> Losses
          </p>
          <p className="text-xl font-bold text-white">{user.losses}</p>
        </div>
        <div className="bg-neutral-900 p-3 rounded-xl border border-neutral-800 flex flex-col justify-center col-span-2 sm:col-span-1">
          ...
          <p className="text-xl font-bold text-white">{user.games_played}</p>
        </div>
      </div>
    </Card>
  );
