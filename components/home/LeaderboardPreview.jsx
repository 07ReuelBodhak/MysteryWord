  ...
  return (
    <Card className="flex flex-col gap-6 bg-neutral-900/30">
      <div className="flex items-center justify-between">
        ...
        <Link
          href="/leaderboard"
          className="text-[10px] font-bold text-neutral-400 hover:text-white transition-colors uppercase tracking-widest bg-neutral-900 rounded px-2 py-1"
        >
          View All
        </Link>
      </div>
        ...
        {leaderboard.slice(0, 3).map((user, idx) => (
          ...
          >
            <div className="flex items-center gap-3">
              <span className="text-neutral-500 font-bold w-4 text-sm">
                {idx + 1}
              </span>
              <Avatar
                src={user.avatar_url}
                size="sm"
                className="w-8 h-8 rounded-full bg-neutral-800 border-neutral-700 font-bold"
              />
