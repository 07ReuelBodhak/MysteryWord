...
import { getProfile, getRecentGames } from "@/lib/api";
  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        if (profileData?.user?.discord_id) {
          console.log("i got the discord id");

          const games = await getRecentGames(profileData.user.discord_id);
